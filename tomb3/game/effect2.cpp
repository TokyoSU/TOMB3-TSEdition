...
void TriggerWeaponSmokeAndShell(ITEM_INFO* item, BITE_INFO* bite, long power)
{
	short fx_num = CreateEffect(item->room_number);
	if (fx_num != NO_ITEM)
	{
		PHD_VECTOR pos = {};
		pos.x = bite->x >> 2;
		...
		GetJointAbsPosition(item, &pos, bite->mesh_num);
		TriggerGunSmoke(pos.x, pos.y, pos.z, 0, 0, 0, 0, LG_PISTOLS, power);
	}
}
