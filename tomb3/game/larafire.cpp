...
void LaraGetNewTarget(WEAPON_INFO* winfo)
{
	...
	for (item_number = next_item_active; item_number != NO_ITEM; item_number = item->next_active)
	{
		...
		// NOTE: Avoid the turret to be targeted if the AI_GUARD is found,
		// that avoid lara to attack friend !
		if (item->object_number == ROBOT_SENTRY_GUN && item->item_flags[1] != 0)
			continue;
