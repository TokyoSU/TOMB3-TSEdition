...
void CreatureAIInfo(ITEM_INFO* item, AI_INFO* info, bool targetLaraByDefault, short maxDetectAngle)
{
	CREATURE_INFO* creature;
	OBJECT_INFO* obj;
	ITEM_INFO* enemy;
	...
	if (enemy == NULL)
	{
		if (targetLaraByDefault)
		{
			enemy = lara_item;
			creature->enemy = lara_item;
		}
		else return; // No target found, quit to avoid crash !
	}

	if (creature->LOT.fly)
		zone = ground_zone[0][0];
	else
...
void CreatureJoint(ITEM_INFO* item, short joint, short required, short maxDiffAngle, short maxRotAngle)
{
	...
	if (change > maxDiffAngle)
		change = maxDiffAngle;
	else if (change < -maxDiffAngle)
		change = -maxDiffAngle;
...
ITEM_INFO* GetNearTarget(ITEM_INFO* item, int radius, int type)
{
	ITEM_INFO* candidate = NULL;
	ITEM_INFO* target = NULL;
	long best_dist = INT_MAX;
	GetNearByRooms(item->pos.x_pos, item->pos.y_pos, item->pos.z_pos, radius, 0, item->room_number);

	for (int i = 0; i < number_draw_rooms; i++)
	{
		for (short n = room[draw_rooms[i]].item_number; n != NO_ITEM; n = candidate->next_item)
		{
			candidate = &items[n];
			if (candidate->object_number == type && candidate->hit_points > 0 && candidate->status == ITEM_ACTIVE)
			{
				long x = (candidate->pos.x_pos - item->pos.x_pos) >> 6;
				long y = (candidate->pos.y_pos - item->pos.y_pos) >> 6;
				long z = (candidate->pos.z_pos - item->pos.z_pos) >> 6;
				long dist = SQUARE(x) + SQUARE(y) + SQUARE(z);
				if (dist < best_dist)
				{
					target = candidate;
					best_dist = dist;
