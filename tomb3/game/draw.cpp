...
static BITE_INFO EnemyBites[16] =	//bite_offsets enum
{
	...
	{ 0, 300, 56, 7 },
	{ 0, 200, 106, 7 },
	{ 110, -30, -530, 2 }, // AUTOGUN_LEFT_BITE
	{ -110, -30, -530, 2 }, // AUTOGUN_RIGHT_BITE
	{ 0, 300, 56, 7 }
};
...
void DrawAnimatingItem(ITEM_INFO* item)
{
	...
	if (frac)
	{
		...
		for (int i = 0; i < obj->nmeshes - 1; i++, bone += 4, meshpp++)
		{
			...
			for (int j = 0; j < 2; j++)
			{
				if (obj->bite_offset[j] == NO_BITE_OFFSET)
					continue;

				if (item->fired_weapon[j] > 0 && i == EnemyBites[obj->bite_offset[j]].mesh_num - 1)
				{
					bite = &EnemyBites[obj->bite_offset[j]];
					...
					if (EnemyWeapon[obj->bite_offset[j]] & 1)
						phd_PutPolygons(meshes[objects[M16_FLASH].mesh_index], clip);
					else
						phd_PutPolygons(meshes[objects[GUN_FLASH].mesh_index], clip);
				...
				if (i == (EnemyBites[obj->bite_offset[j]].mesh_num - 1) && EnemyWeapon[obj->bite_offset[j]] & 0x80)
				{
					if (item->hit_points > 0 || item->frame_number != anims[item->anim_number].frame_end)
					{
						bite = &EnemyBites[obj->bite_offset[j] + 1];
						pos.x = bite->x;
						pos.y = bite->y;
						pos.z = bite->z;
